...
class Modelo_cobros extends CI_Model{
	...
	function listarTodos($fecha_inicio, $fecha_final){
		$cobros = $this->db->query('select c.id_cobro, CONCAT( c.id_user,". ",up.nombre," ", up.apellido) usuario, cm.descripcion metodo_pago, cs.descripcion estado, round((c.monto*0.9),2) monto, round((c.monto*0.1),2) retencion, c.fecha, c.fecha_pago ,c.cuenta, c.banco, c.titular, c. otro
	...
	function ConsultarCobrosFecha($fecha_inicio, $fecha_final){
		$cobros = $this->db->query("select 
									...
									from
									    ...
									    cat_estatus cs
									where
									    c.id_user = up.user_id
									        and c.id_metodo = cm.id_metodo
									        and c.id_estatus = cs.id_estatus
									        and c.id_estatus = 3
									        and c.fecha BETWEEN '".$fecha_inicio." 00:00:00' 
...
from cobro c, user_profiles up, cat_metodo_cobro cm, cat_estatus cs
where c.id_user = up.user_id and c.id_metodo = cm.id_metodo and c.id_estatus = cs.id_estatus and c.id_estatus = "DES"');
		return $cobros->result();
	}
