...
class reportes extends CI_Controller {
	...
	function reportes_tipo() {
		switch ($_POST ['tipo']) {
			...
			case 7 :
				...
				break;
			case 8 :
				$this->reporte_afiliados_mes ();
				break;
			case 9 :
				$this->reporte_cobros_pendientes ();
				break;
			case 10 :
				$this->reporte_comisiones_pagadas ();
				break;
			case 11 :
				$this->reporte_cobros_todos ();
				break;
			case 12 :
				$this->reporte_cobros_pagados ();
				break;
			case 13 :
				$this->reporte_comisiones_por_pagar ();
				break;
			case 14 :
				$this->reporte_ventas_pagadas_online ();
				break;
			case 15 :
				$this->afiliados_activos ();
				break;
			case 16 :
	...
	function reporte_bonos_afiliados_todos() {
		...
		foreach ( $bonos as $bono ) {
				...
				echo "<tr>" . "<td class='sorting_1'>" . $bono->id . "</td>" 
						. "<td>" . $bono->usuario . "</td>" 
						. "<td>" . $bono->afiliado . "</td>"								
						. "<td> " . $bono->fecha . "</td>" 
						. "<td> " . $bono->bono . "</td>" 								
	...
	function reporte_todo_comisiones() {
		...
		echo "<tr>" 
					...
					. "<td>$ " . $totales['ventast'] . "</td>"
					. "<td>$ " . $totales['comisiones'] . "</td>"
					. "<td>$ " . $totales['bonos'] . "</td>"
					. "<td>$ " . $totales['comisionest'] ."</td>"
					. "<td>$ " . $totales['transacciones'] . "</td>"
					. "<td>$ " . $totales['pagos'] ."</td>"
					. "<td>$ " . $totales['neto'] ."</td>" 
				. "</tr>";
	...
	function reporte_comisiones_por_pagar() {
		...
		for($i = 0; $i < sizeof ( $cobros ); $i ++) {
			...
			<td>" . $cobros [$i]->titular . "</td>
			<td>" . $cobros [$i]->otro . "</td>
			<td>$ " . number_format ( $cobros [$i]->retencion, 2 ) . "</td>
			<td>$ " . number_format ( $cobros [$i]->monto, 2 ) . "</td>
			</tr>";
			$totalCobros += $cobros [$i]->monto;
                        $totalRetencion += $cobros [$i]->retencion;
		}
		...
		echo "<tr>
			...
			<td><b></b></td>
			<td><b></b></td>
			<td><b> $	" . number_format ( $totalRetencion, 2, '.', '' ) . "</b></td>
			<td><b> $	" . number_format ( $totalCobros, 2, '.', '' ) . "</b></td>
			</tr>";
	...
	function reporte_comisiones_por_pagar_excel() {
		...
		for($i = 0; $i < sizeof ( $cobros ); $i ++) {
			...
			$this->excel->getActiveSheet ()->setCellValueByColumnAndRow ( 7, ($i + 8), $cobros [$i]->otro );
			$this->excel->getActiveSheet ()->setCellValueByColumnAndRow ( 8, ($i + 8), $cobros [$i]->retencion );
			$this->excel->getActiveSheet ()->setCellValueByColumnAndRow ( 9, ($i + 8), $cobros [$i]->monto );
			$total += $cobros [$i]->monto;
                        $totalR += $cobros [$i]->retencion;
			$ultima_fila = $i + 8;
			$totalCobros ++;
			$contador_filas ++;
		}
		...
		$this->excel->getActiveSheet ()->setCellValueByColumnAndRow ( 8, ($ultima_fila + 2), $totalR );
	...
	function reporte_comisiones_pagadas() {
		...
		$totalCobros = 0;$totalRetencion = 0;
	...
	function reporte_comisiones_pagadas_excel() {
		...
		for($i = 0; $i < sizeof ( $cobros ); $i ++) {
			...
			$this->excel->getActiveSheet ()->setCellValueByColumnAndRow ( 9, ($i + 8), $cobros [$i]->retencion );
		...
		$this->excel->getActiveSheet ()->setCellValueByColumnAndRow ( 9, ($ultima_fila + 2), $totalR );
		$this->excel->getActiveSheet ()->setCellValueByColumnAndRow ( 10, ($ultima_fila + 2), $total );
	...
	function reporte_cobros_todos_excel() {
		...
		for($i = 0; $i < sizeof ( $cobros ); $i ++) {
			...
			$this->excel->getActiveSheet ()->setCellValueByColumnAndRow ( 2, ($i + 8), $cobros [$i]->fecha );
			$this->excel->getActiveSheet ()->setCellValueByColumnAndRow ( 3, ($i + 8), $cobros [$i]->fecha_pago );
			$this->excel->getActiveSheet ()->setCellValueByColumnAndRow ( 4, ($i + 8), $cobros [$i]->otro );
			$this->excel->getActiveSheet ()->setCellValueByColumnAndRow ( 5, ($i + 8), $cobros [$i]->banco );
			$this->excel->getActiveSheet ()->setCellValueByColumnAndRow ( 6, ($i + 8), $cobros [$i]->cuenta );
			$this->excel->getActiveSheet ()->setCellValueByColumnAndRow ( 7, ($i + 8), $cobros [$i]->titular );
			$this->excel->getActiveSheet ()->setCellValueByColumnAndRow ( 8, ($i + 8), $cobros [$i]->retencion );
    ...
    public function reporte_todo_bonos_excel() {
	...
	$contador_filas = 0;
        ...
        foreach ($bonos as $ibono){
            ...
            $this->table = array(
                $ibono->id,
                $ibono->afiliado."(".$ibono->id_usuario.")",
                $ibono->fecha,
                $ibono->bono
            );   
            
            $this->profundizar_bono_excel($ibono->id_usuario, $ibono->id_bono, "'".$ibono->fecha."'", $ibono->valor);
            
        }
        
        $this->model_excel->setTemplateExcelReport ( "Comisiones Pagadas", $cabeza, $this->contador_filas, $this->excel );
        
		$filename = 'Bonos_Pagados_Detalle_de' . $inicio . ' al ' . $fin . '.xls'; // save our workbook as this file name
		header ( 'Content-Type: application/vnd.ms-excel' ); // mime type
		header ( 'Content-Disposition: attachment;filename="' . $filename . '"' ); // tell browser what's the file name
		header ( 'Cache-Control: max-age=0' ); // no cache
    ...
    function profundizar_bono_excel($id,$id_bono,$fecha,$valor){
    ...
    function BonoTableExcel($datos){
        
        foreach($datos as $dato){  
                    $table2 = $this->table;
                    //var_dump($this->table);                  
                    foreach ($dato as $key => $value){
                        array_push($table2,$value);
                        //echo $value."|";
                    ...
                    //echo "<br/>";
                    $i=0;
                    foreach ($table2 as $dato){
                        
                        $this->excel->getActiveSheet()->setCellValueByColumnAndRow ( $i, ($this->contador_filas + 8), $dato );
                        $i++;
