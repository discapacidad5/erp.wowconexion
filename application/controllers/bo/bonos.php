...
class bonos extends CI_Controller
{
	...
	function pagar_bono_calculado(){
		...
		$id_bono=$_POST['id_bono'];
                ...
                $isPago = $this->calculador_bono->calcularComisionesPorBono($id_bono,$fecha);
	...
	function detalle_historial(){
		...
		foreach($historial as $hist)
		{
			...
			echo "<tr>
			<td class='sorting_1'>".$hist->id_usuario."&nbsp;<a style='cursor: pointer' title='Ver Detalles' onclick='profundizar(".$hist->id_usuario.",".$hist->id_bono.",`".$hist->fecha."`,".$hist->valor.")'><i class='fa fa-eye'></i><a></td>
			<td>".$hist->nombres."</td>
			<td>".$hist->bono."</td>
			<td>".$hist->dia."</td>
			<td>".$hist->mes."</td>
		...
		echo "</tbody>
                ...
                //escribir funcion js
                echo  "<script>"
                                .   "function profundizar(usuario,id,fecha,valor){"
                                .   "   $.ajax({
                                            type: 'POST',
                                            url: '/ov/billetera2/profundizar_bono',
                                            data: {id: id,usuario: usuario,fecha: fecha,valor: valor}
                                        }).done(function( msg ){					
                                                bootbox.dialog({
                                                    message: msg,
